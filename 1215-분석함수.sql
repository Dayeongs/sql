...
TOP-N 분석
    ...
    형식
        ...
        * ORDER BY절로 데이터를 정렬한 후 ROWNUM으로 몇개의 데이터만 추출해야 하는 경우, 
          ORDER BY절을 먼저 처리하도록 인라인 뷰를 만들고,그 뷰를 감싸는 바깥쪽 SQL에서 ROWNUM 처리를 하자.
*/

-- 직원들을 급여순으로 정렬했을 때 급여를 가장 많이 받는 직원 5명(1등 ~ 5등)의 
-- 직원아이디, 이름, 급여, 직종아이디를 조회하기
...
-- 아래코드는 TOP-N 분석은 아니다.
-- 직원들을 급여순으로 정렬했을 때 급여를 가장 많이 받는 직원 5명(6등 ~ 10등)의 
-- 직원아이디, 이름, 급여, 직종아이디를 조회하기
...
ROW_NUMBER 함수
     ...
     ROW_NUMBER 함수를 활용하면 특정 범위내의 데이터를 조회할 때 유리하다.
*/  
...
-- SELECT SUM() -> 그룹함수, 결과 1개 반환
-- SELECT SUM() OVER () -> 분석함수, 행마다 결과 반환
