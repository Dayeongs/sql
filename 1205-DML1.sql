...
-- 직원테이블에서 60번 부서에 소속된 직원들의 아이디, 이름, 급여를 조회하고,
...
-- 직원테이블에서 50번 부서에 속소된 직원들의 직원아이디, 이름, 입사일, 급여를 조회하고,
...
-- 직원테이블에서 직원들의 직원아이디, 이름, 급여, 부서아이디를 조회하고,
...
/*
    ...
    3. 기타연산자
        ...
        * WHERE절에서 지정된 컬럼의 값이 NULL인 아닌 행을 조회한다.
        ...
        * '자바', '자바의 정석', '자바 프로그래밍'는 전부 위의 패턴과 일치한다.
        ...
        WHERE BOOK_TITLE LIKE '%자바'
        * '자바', '일주일안에 끝내는 자바', '이펙티브 자바'는 전부 위의 패턴과 일치한다.
        ...
        * '자바', '자바의 정석', '이것이 자바다', '이펙티브 자바'는 전부 위의 패턴과 일치한다.
        ...
        WHERE NAME LIKE '김_%'
        * '김'은 위의 패턴과 일치하지 않는다.
        * '김구', '김유신', '김수한무거북이와두루미'는 정부 위의 패턴과 일치한다.
...
FROM EMPLOYEES
WHERE HIRE_DATE BETWEEN '2005/01/01' AND '2025/12/31'
ORDER BY HIRE_DATE ASC;
...
-- 부서테이블에서 관리자가 지정된 부서아이디와 부서이름, 관리자 아이디를 조회하기
