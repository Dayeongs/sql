...
-- 80번 부서에 소속된 직원들의 직원아이디, 이름, 직종제목, 급여, 직종의 최고급여와 급여간의 차이를 조회하기(등가조인)
...
-- 서브쿼리
-- 조회조건(이름)이 고유한 값이 아니기때문에 IN을 사용한다. (동명이인이 있을 수 있다.)
...
-- 부서별 평균급여를 계산했을 때 'Ismael'이 근무하는 부서의 평균급여보다 급여를 많이 받는 부서의 
-- 부서 아이디와 평균급여를 조회하기(GROUP BY, HAVING, 서브쿼리)
